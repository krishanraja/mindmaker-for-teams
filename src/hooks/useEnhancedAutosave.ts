...
export function useEnhancedAutosave<T>({
  ...
  const forceSave = useCallback(async () => {
    ...
    const saveWithRetry = async (attemptsLeft: number = maxRetries): Promise<void> => {
      try {
        ...
        console.error(`[${componentName}] Force save error (${maxRetries - attemptsLeft + 1}/${maxRetries}):`, saveError);
    ...
    await saveWithRetry();
  }, [data, saveFunction, componentName, maxRetries, clearSave]);
  ...
  return { isSaving, lastSaved, error, retryCount, forceSave };
}
